...
function __construct(){
	...
	public function login(){
		...
		if($this->form_validation->run() != false){
			...
			$data = $this->M_ikm->edit_data($where,'admin');
			$d = $this->M_ikm->edit_data($where,'admin')->row();
			$cek = $data->num_rows();
			...
			if($cek > 0)
			{
				...
				$dt = $this->M_ikm->edit_data($where, 'anggota');
				$hasil = $this->M_ikm->edit_data($where, 'anggota')->row();
				$proses = $dt->num_rows();
